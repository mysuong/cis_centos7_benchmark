...
# 5.5.4 - Ensure default user shell timeout is configured (Automated) - Server1 Workstation1
...
for f in "$BRC" /etc/profile /etc/profile.d/*.sh ; do
	...
	grep -Pq '^\s*([^#]+;\s*)?export\s+TMOUT(\s+|\s*;|\s*$|=(900|[1-8][0-9][0-9]|[1-9][0-9]|[1-9]))\b' "$f" && output1="$f"
done
...
if [ -n "$output1" ] && [ -z "$output2" ]; then
	...
	[ -n "$output2" ] && echo -e "\nFAILED\n\nTMOUT is incorrectly configuredin: \"$output2\"\n"
exit 1
fi